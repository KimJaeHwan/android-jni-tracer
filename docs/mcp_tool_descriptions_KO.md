...
반환값: manifest 객체 구성:
  ...
  - termination_kind (str|null): 비정상 종료 감지 시 "timeout", "nonzero_exit", "signal_<n>"
  - summary_source (str|null): 정상 파싱이면 "jni_hook.json", 복구 요약이면 "jni_hook.ndjson"
  - partial_log_available (bool): 실행 실패여도 구조화 로그 일부가 복구되었는지 여부
  - log_parse_error (str|null): jni_hook.json 잘림/손상 시 기록되는 JSON 파싱 오류
...
중요한 실패 처리 메모:
  run이 status="failed"여도 증거가 완전히 사라진 것은 아닙니다.
  manifest.partial_log_available 가 true 라면 다음 필드를 함께 보세요:
    - manifest.termination_kind
    - manifest.summary_source
    - manifest.log_parse_error
  즉, 하네스나 타깃이 비정상 종료되었지만 부분 구조화 로그는 복구되어
  여전히 분석에 사용할 수 있다는 뜻입니다.
...
크래시/kill 해석:
  특정 단계가 크래시성 경로를 유발하면 run은 다음처럼 반환될 수 있습니다:
    - status="failed"
    - returncode != 0
    - manifest.termination_kind = "signal_<n>"
  이것을 무조건 "도구 실패"로 해석하면 안 됩니다.
  manifest.partial_log_available 가 true 라면 부분 실행 trace로 취급하고,
  마지막으로 관측된 InvokeNative / Call*Method 이벤트를 분석하세요.
