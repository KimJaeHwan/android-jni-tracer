...
- `manifest.json`에서 `termination_kind`, `summary_source`,
  `partial_log_available`, `log_parse_error`를 확인해야 한다.
- `status="failed"`여도 `partial_log_available=true`이면
  타깃 비정상 종료를 포함한 부분 trace를 계속 분석할 수 있다.
...
- 비정상 종료 시 NDJSON recovery summary 생성 가능
